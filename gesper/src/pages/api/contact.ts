...
function isAllowedOrigin(request: Request) {
  ...
  const allowedOrigins = new Set([
    ...
    'https://gesper.noanet.com.ar',
    'https://www.gesper.noanet.com.ar',
  ]);
...
function parseContactPayload(payload: ContactPayload) {
  const data: ContactFormData = {
    ...
    employees: clean(payload.employees, limits.employees),
    industry: clean(payload.industry, limits.industry),
    role: clean(payload.role, limits.role),
  ...
  if (!data.name || !data.company || !data.employees || !data.industry || !data.role || !data.phone || !data.email) {
    return { error: 'required_fields' as const };
  }

  const employees = Number.parseInt(data.employees, 10);

  if (!Number.isInteger(employees) || employees < 1 || employees > 1_000_000) {
    return { error: 'invalid_employees' as const };
  }

  data.employees = String(employees);
